...
local n = 1
foreach vars of varlist MDDW MDDW_purch MDDW_own MDDW_rec {
...
foreach var of varlist exp_ae_w_ln exp_ae_w_ln_sq education1 education3 education4 hhsize age_hhh age_hhh_sq occup_agr_any sex_hhh urban {
	...
	local textinput "`textinput' `n'.15 -2.0 "SD{subscript:1}=`sd3'" `n'.35 -2.0 "SD{subscript:2}=`sd4'""
	
	local n = `n' + 1
}

local n = 1
foreach var of varlist sd_1_MDDW sd_2_MDDW_purch sd_3_MDDW_own sd_4_MDDW_rec {
...
coefplot ///
	(reg1_stdout, label(MDDW (SD=`sd1')) mcolor(midgreen) ciopts(lcolor(midgreen)) ) ///
	(reg2_stdout, label(MDDW (purchases; SD=`sd2')) mcolor(midblue) ciopts(lcolor(midblue)) ) ///
	(reg3_stdout, label(MDDW (own consumption; SD=`sd3')) mcolor(sienna) ciopts(lcolor(sienna)) ) ///
	(reg4_stdout, label(MDDW (gifts/in-kind; SD=`sd4')) mcolor(gold) ciopts(lcolor(gold)) ) ///
	, keep(MDDW* exp_ae_w_ln_std *exp_ae_w_ln_sq_std *education?_std hhsize_std age_hhh_std age_hhh_sq_std *sex_hhh_std *urban_std *occup_agr_any_std) ylab(,labsize(7pt)) xlab(,labsize(7pt)) xline(0, lcol(black) lpattern(dash)) graphregion(color(white))  ///
	mlabel(cond(@pval<.001, string(@b,"%9.3f") + "***", cond(@pval<.01, string(@b,"%9.3f") + "**", cond(@pval<.05, string(@b,"%9.3f") + "*", string(@b,"%9.3f"))))) ///
	format(%9.2f) mlabposition(3) mlabgap(*6) mlabsize(5pt) msize(small) mlabcolor(black) msymbol(O) ysize(16cm) xsize(15cm) ///
	legend(size(6pt) rows(2)) ///
    coeflabels(                                            ///
...
graph export "${workdir}/graphs/MDDW_reg.png", replace
graph export "${workdir}/graphs/MDDW_reg.pdf", replace
...
* open workbook
putexcel set "${workdir}/tables/partialR2_table_mddw.xlsx", replace sheet("DeltaR")

* header
putexcel A1=("Variable") B1=("MDDW") C1=("Purchases") D1=("Own cons.") E1=("Gifts-in-kind")
