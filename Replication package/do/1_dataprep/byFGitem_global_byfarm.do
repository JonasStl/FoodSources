...
* Calculate shares
forv z = 1/12 {
	gen Ncons_shr_purch_`z' = (HDDS_`z'_Ncons_purch/HDDS_`z'_Ncons)*100
	gen Ncons_shr_own_`z' = (HDDS_`z'_Ncons_own/HDDS_`z'_Ncons)*100
	gen Ncons_shr_rec_`z' = (HDDS_`z'_Ncons_rec/HDDS_`z'_Ncons)*100
}
...
foreach var in purch own rec {
	...
	forv v = 1/12 {
		...
		svy, subpop(insample): mean Ncons_shr_`var'_`v', over(occup_agr_any)
		/*
		Sample:
		- 
		*/ 
		...
		mat define C`n'_`v' = J(2,1,e(N))
		mat define M`n'_`v' = B`n'_`v'[1..2,1..2], B`n'_`v'[1..2,5..6], C`n'_`v'
	}
...
lab var occup_agr_any "Household agricultural status"
...
lab var se_Ncons_shr_purch "Standard Error (consumption from purchases)"
...
lab var se_Ncons_shr_own "Standard Error (consumption from own production)"
...
lab var se_Ncons_shr_rec "Standard Error (consumption from gifts/in-kind)"
...
save "$datadir/graphs and tables/byFGitem_global_byfarm.dta", replace
